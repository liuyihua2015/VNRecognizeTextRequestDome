...
//            [detectRequest setValue:@(YES) forKey:@"reportCharacterBoxes"]; // 设置识别具体文字
            [self recognizeText:DSDetectionTypeTextRectangles image:image complete:complete];
...
/// 文字识别
+ (void)recognizeText:(DSDetectionType)type image:(UIImage *_Nullable)image complete:(detectImageHandler _Nullable )complete
{
    ...
    if (@available(iOS 13.0, *)) {
        ...
        VNRecognizeTextRequest * textRecognitionRequest = [[VNRecognizeTextRequest alloc]initWithCompletionHandler:^(VNRequest * _Nonnull request, NSError * _Nullable error) {
            ...
            NSMutableArray *textsArray = @[].mutableCopy;
...
//                NSString * transcript = recognizedText.string;
//                transcript = [transcript stringByAppendingString:@"\n"];
                [textsArray addObject:recognizedText.string];
            }
            ...
            if (complete) {
                complete(detectTextData,textsArray);
            }
        ...
        textRecognitionRequest.recognitionLanguages = @[@"zh-Hant"];
        
        
        VNImageRequestHandler * imageRequestHandler = [[VNImageRequestHandler alloc]initWithCGImage:image.CGImage options:@{}];
